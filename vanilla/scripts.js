...
const changeTexts = (lang) => {
	// console.log(lang);
};

let navigatorLang = navigator.language || navigator.userLanguage;
navigatorLang = navigatorLang.includes('ca') ? 'ca' : navigatorLang.includes('es') ? 'es' : 'en';

const usrLang = localStorage.getItem('userLang') ? localStorage.getItem('userLang') : navigatorLang;
...
document.querySelector('#mode').addEventListener('click', () => {
	localStorage.setItem('usrMode', localStorage.getItem('usrMode') === 'light' ? 'dark' : 'light');
